...
import { 
  ...
  FileText, AlertCircle, Copy, Zap, Lock, Users, Share2, ExternalLink
} from 'lucide-react';
...
// Xaman Configuration
const XAMAN_API_KEY = import.meta.env.VITE_XAMAN_API_KEY || 'acc89ff2-800c-4e09-84ee-ef2837e398ea';
...
// Xaman Connection Modal
const XamanModal = ({ onClose, qrUrl, deepLink }) => (
  <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
    <Card className="p-8 max-w-md w-full">
      <div className="text-center">
        ...
        <h2 className="text-2xl font-bold text-white mb-2">Connect with Xaman</h2>
        <p className="text-gray-400 mb-6">Scan QR code or open in Xaman app</p>
        
        {qrUrl && (
          <div className="bg-white p-4 rounded-lg mb-6">
            <img src={qrUrl} alt="Xaman QR Code" className="w-full h-auto" />
          </div>
        )}
        ...
        <p className="text-gray-500 text-xs mt-4">
          Don't have Xaman? <a href="https://xaman.app" target="_blank" rel="noopener noreferrer" className="text-cyan-400 hover:underline">Download here</a>
        </p>
      </div>
    </Card>
  </div>
      ...
      {wallet ? (
        <div className="flex items-center gap-3">
          <div className="hidden sm:flex items-center gap-2 px-3 py-2 bg-green-500/10 border border-green-500/30 rounded-lg">
            ...
            <span className="text-green-400 text-sm font-semibold">Xaman Connected</span>
          </div>
        ...
        <Button variant="primary" onClick={connectWallet} disabled={loading} icon={Wallet}>
          {loading ? 'Connecting...' : 'Connect Xaman'}
        </Button>
      )}
    </div>
  </header>
    ...
    <section className="container mx-auto px-4 py-20 md:py-32">
      <div className="text-center max-w-5xl mx-auto">
        <span className="inline-block px-4 py-2 bg-cyan-500/10 border border-cyan-500/30 rounded-full text-cyan-400 text-sm font-semibold mb-6">
          Built on XRP Ledger • Powered by Xaman
        </span>
        ...
        <p className="text-xl text-gray-300 mb-12 max-w-3xl mx-auto">
          OLOPA is the trust layer for P2P agreements on XRPL. Connect with Xaman for secure, non-custodial escrow.
        </p>
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          <Button variant="primary" size="lg" icon={wallet ? ArrowRight : Wallet} onClick={() => wallet ? setView(ROUTES.CREATE) : connectWallet()}>
            {wallet ? 'Create Your First Deal' : 'Connect Xaman to Start'}
          </Button>
      ...
      <div className="container mx-auto px-4 max-w-4xl">
          ...
          <div className="space-y-4 border-t border-slate-700 pt-6">
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-center gap-2">
                <span className="text-white font-mono text-sm">{deal.creator.slice(0, 10)}...{deal.creator.slice(-6)}</span>
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-center gap-2">
                <span className="text-white font-mono text-sm">{deal.freelancerAddress.slice(0, 10)}...{deal.freelancerAddress.slice(-6)}</span>
            ...
            {deal.escrowTxHash && (
              <div className="flex justify-between">
                ...
                <a href={`https://testnet.xrpl.org/transactions/${deal.escrowTxHash}`} target="_blank" rel="noopener noreferrer" className="text-cyan-400 hover:underline font-mono text-sm">
          ...
          <Card className="p-6 mb-6 bg-blue-900/20 border-blue-500/30">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-gray-300 text-sm mb-4">Send this link to the freelancer:</p>
        ...
        {userRole === DEAL_ROLES.CLIENT && !clientSigned && (
          <Card className="p-6 mb-6">
            <h3 className="text-xl font-bold text-white mb-4">Sign Agreement with Xaman</h3>
            <p className="text-gray-300 mb-4">Sign this deal via Xaman to confirm the terms.</p>
            <Button variant="primary" onClick={signDeal} disabled={loading} icon={Wallet}>
              {loading ? 'Signing...' : 'Sign with Xaman'}
            </Button>
          </Card>
        )}
        ...
        {userRole === DEAL_ROLES.FREELANCER && !freelancerSigned && (
          <Card className="p-6 mb-6">
            <h3 className="text-xl font-bold text-white mb-4">Review & Sign with Xaman</h3>
            <p className="text-gray-300 mb-4">Review the terms and sign via Xaman to accept.</p>
            <Button variant="primary" onClick={signDeal} disabled={loading} icon={Wallet}>
              {loading ? 'Signing...' : 'Sign with Xaman'}
            </Button>
          </Card>
        ...
        {userRole === DEAL_ROLES.CLIENT && bothSigned && deal.status === DEAL_STATUS.SIGNED && (
          <Card className="p-6 mb-6">
            <h3 className="text-xl font-bold text-white mb-4">Fund Escrow via Xaman</h3>
            <p className="text-gray-300 mb-4">Lock {deal.amount} {deal.currency} in XRPL escrow using Xaman.</p>
            <Button variant="success" icon={Lock} onClick={fundEscrow} disabled={loading}>
              {loading ? 'Creating Escrow...' : 'Fund Escrow with Xaman'}
            </Button>
          </Card>
        )}
          ...
          <Card className="p-6 bg-green-900/20 border-green-500/30">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-gray-300 text-sm mt-1">Secured on XRPL via Xaman. Auto-releases after deadline.</p>
              </div>
            </div>
          </Card>
        )}
...
export default function App() {
  ...
  const [xamanModal, setXamanModal] = useState(null);
  
  const [formData, setFormData] = useState({
    title: '', amount: '', currency: 'XRP', freelancerAddress: '', deadline: '7'
  });
  ...
  useEffect(() => {
    ...
    if (dealId) loadDealFromUrl(dealId);
  }, []);
  ...
  const loadDealFromUrl = (dealId) => {
    const mockDeal = {
      id: dealId, title: 'Website Development', amount: '500', currency: 'XRP',
      creator: 'rN7n7otQDd6FczFgLdlqtyMVrn3HMtthP4',
      freelancerAddress: 'rLHzPsX6oXkzU9fNZmPEd7ks9TKmfD1Fqv',
      deadline: '7', status: DEAL_STATUS.PENDING,
    ...
    try {
      ...
      if (data.uuid) {
        ...
        const checkPayload = async () => {
          const statusRes = await fetch(`https://xumm.app/api/v1/platform/payload/${data.uuid}`, {
          ...
          if (status.meta.signed) {
            ...
            setXamanModal(null);
            setLoading(false);
          } else if (status.meta.resolved && !status.meta.signed) {
            setXamanModal(null);
            setLoading(false);
            alert('Connection cancelled');
          } else {
            setTimeout(checkPayload, 2000);
          }
        };
      ...
      setLoading(false);
      alert('Failed to connect to Xaman');
    }
  }, []);
  ...
  const createDeal = useCallback(() => {
    ...
    setLoading(true);
    const deal = {
      id: `DEAL-${Date.now()}`, ...formData, status: DEAL_STATUS.PENDING,
      creator: wallet.address, createdAt: new Date().toISOString(),
      signatures: { client: false, freelancer: false }
    };
    setTimeout(() => {
      setCurrentDeal(deal);
  ...
  const signDeal = useCallback(() => {
    ...
    setLoading(true);
    // In production: Create Xaman payload for signature
    setTimeout(() => {
      const updated = { ...currentDeal };
      if (userRole === DEAL_ROLES.CLIENT) updated.signatures.client = true;
      else if (userRole === DEAL_ROLES.FREELANCER) updated.signatures.freelancer = true;
      if (updated.signatures.client && updated.signatures.freelancer) updated.status = DEAL_STATUS.SIGNED;
  ...
  const fundEscrow = useCallback(() => {
    ...
    setLoading(true);
    // In production: Create Xaman EscrowCreate payload
    setTimeout(() => {
      setCurrentDeal(prev => ({
        ...prev, status: DEAL_STATUS.FUNDED,
    ...
    <div className="min-h-screen bg-slate-900 text-white">
      <div className="bg-gradient-to-r from-yellow-500 to-orange-500 text-black px-4 py-3 text-center sticky top-0 z-50">
        <div className="flex items-center justify-center gap-2 text-sm font-semibold">
          ...
          <span>BETA: Non-custodial. Powered by Xaman. Use at your own risk.</span>
        </div>
      </div>

      {xamanModal && <XamanModal onClose={() => setXamanModal(null)} qrUrl={xamanModal.qrUrl} deepLink={xamanModal.deepLink} />}

      <Header wallet={wallet} loading={loading} connectWallet={connectWallet} disconnectWallet={disconnectWallet} setView={setView} />
